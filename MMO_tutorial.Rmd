...
This tutorial is meant to accompany @smith_modelled_2024 ([manuscript](https://www.isca-archive.org/interspeech_2024/smith24_interspeech.pdf)), which describes a new method for calculating vowel merger: Modelled Multivariate Overlap (MMO). This tutorial demonstrates the method by walking through an example analyzing the PIN-PEN merger in Raleigh, NC [@labov_atlas_2006]. We operationalize the degree of merger between two vowels as the amount of distributional overlap in F1 / F2 space between those two vowels. While the paper and the tutorial both focus on vowel merger, the method applies more generally to quantifying the overlap between distributions of any kind, including distributions in more than 2 dimensions[^1]. Additionally, this method is flexible in that it can be applied for any measure of overlap that can be calculated from empirical distributions. Here, we show the method applied to calculate Bhattacharyya affinity and Euclidean distance, but it could also be used to calculate Pillai scores or any of the other methods described in @nycz_best_2014 or @kelley_comparison_2020.
...
```
We can see that both on average and for each individual speaker, there appears to be an interaction between `vowel` and `context` F1 slope, but not for F2 slope. We expect our fitted model to reflect this. However, because the interaction between `vowel` and `context` is of fundamental interest here, we must include it in the model regardless of whether or not the empirical plots suggest we should. 

## Model fitting {#Model_Fitting}

We are going to fit a simple multivariate Bayesian model (similar to the "minimal" model in our Interspeech paper [@smith_modelled_2024]). For more model syntax examples (including more complex model structures, such as nested random effects, modelling variance, and predictors with nonlinear effects), see the model-fitting scripts in the `Interspeech2024` folder of the github repository.
...
Each row in `fitted_sp` and `fitted_av` should be interpreted as a possible value of the mean. Because we have 100 such values, we can use this variation to estimate our uncertainty in an estimate of the mean (or of any quantity calculated from these dataframes). In the next section, we will use `fitted_sp` and `fitted_av` to calculate Euclidean distance.

Now, instead of simulating the means, let's simulate the *data*, which is what we need in order to calculate distributional overlap. For this, we use `add_predicted_draws`, but aside from that, the procedure is the same as above. The following two dataframes are not themselves used to calculate a measure of overlap, but these are the dataframes that are directly comparable to the empirical distributions, so we do want to look at them.
...
Setting aside the shape of the predicted category distributions, which are constrained by the model's definition, the distribution centers (means) are roughly comparable across the modelled and empirical distributions. 
...
sp_estimates %>%
  ...
  geom_errorbar(width=.1, aes(ymin=F1.lower, ymax=F1.upper)) +
...
Now that we have predicted and fitted dataframes that resemble the raw data, we can calculate any measure of overlap we like.
...
For calculating Euclidean distance, we need to take a slightly different approach for the empirical distributions than for the modelled distributions. To calculate empirical Euclidean distance, we have to start by taking `speaker`-by-`context`-by-`vowel` averages of for each formant, then calculating the distance between vowels of those averages. 
...
Now that we have by-speaker distributions of BA, let's calculate the by-speaker credible intervals (CIs) for prenasal and preoral BA. We can also calculate the difference between prenasal and preoral BA, and the CI for the difference. To do this, we first take the difference by draw, then calculate the CI based on the distribution of the difference, just like for the BA values.
...
We can also pivot the empirical dataframe for easier plotting (below) and calculate the difference, but there is not a clear way to calculate confidence intervals from the empirical distributions.
...
It is easiest to understand these overlap measures by plotting them. Here, we plot prenasal overlap vs. pre-oral overlap: 
...
We can also make plots showing credible intervals for each speaker.
...
p_BA_oral <- BA_mod_sp_wide %>%
  ...
  geom_point() +
  geom_errorbar(width=0.1, aes(ymin=oral.lower, ymax=oral.upper)) +
  theme(axis.title.x=element_blank())

p_BA_nasal <- BA_mod_sp_wide %>%
  ggplot(aes(x=reorder(speaker, nasal), y=nasal, color=speaker)) +
  geom_point() +
  geom_errorbar(width=0.1, aes(ymin=nasal.lower, ymax=nasal.upper)) +
  theme(axis.title.x=element_blank())

p_BA_diff <- BA_mod_sp_wide %>%
  ggplot(aes(x=reorder(speaker, diff), y=diff, color=speaker)) +
  geom_point() +
  geom_errorbar(width=0.1, aes(ymin=diff.lower, ymax=diff.upper)) +
  xlab('speaker')
